 ...
 */
function isInlineFragmentObject(
    value: unknown,
): value is GraphQLInlineFragment {
    return (
        typeof value === 'object' &&
        value !== null &&
...
function isScalarObject(value: unknown): value is GraphQLScalar {
    return (
        typeof value === 'object' &&
        value !== null &&
        (value as any)[typeSymbol] === GraphQLType.SCALAR
    )
}
...
function renderInlineFragment(
    fragment: GraphQLInlineFragment,
    context: RenderContext,
): string {
    return `...on ${fragment.typeName}${renderObject(
        undefined,
        fragment.internal,
        context,
    )}`
...
function renderType(
    ...
    switch (typeof value) {
        ...
        case 'string':
            throw new Error(
                `Rendering type ${typeof value} directly is disallowed`,
            )
        case 'object':
            // Ignore null fields.
            if (value === null) {
...
function renderObject(
    ...
    return `${renderName(name)}${renderParams(
        (obj as any)[paramsSymbol],
    )}{${fields.join('').trim()}}`
}
...
export function render(value: Record<string, unknown>): string {
    ...
    while (executingContext.fragments.size > 0) {
        ...
        for (const [sym, fragment] of Array.from(
            executingContext.fragments.entries(),
        )) {
